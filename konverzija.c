...
void KonverzijaBCDToBin(unsigned char *k_hours, unsigned char *k_minutes, unsigned char *k_seconds,
                        unsigned char *k_day, unsigned char *k_month, unsigned char *k_year)
{
   *k_seconds  =  ((*k_seconds & 0xF0) >> 4)*10 + (*k_seconds & 0x0F);  // Transform seconds
   *k_minutes  =  ((*k_minutes & 0xF0) >> 4)*10 + (*k_minutes & 0x0F);  // Transform months
   *k_hours    =  ((*k_hours & 0xF0)  >> 4)*10  + (*k_hours & 0x0F);    // Transform hours
   *k_year     =   (*k_day & 0xC0) >> 6;                                // Transform year
   *k_day      =  ((*k_day & 0x30) >> 4)*10 + (*k_day& 0x0F);           // Transform day
   *k_month    =  ((*k_month & 0x10)  >> 4)*10 + (*k_month & 0x0F);     // Transform month
}